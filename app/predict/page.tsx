    ...
    if (targetVariable === "happiness") {
      try {
        ...
        if (data.predicted_score !== undefined) {
          setPrediction(Number.parseFloat(data.predicted_score.toFixed(2)))
          setConfidence(90)
        } else {
          setPrediction(null)
          setConfidence(null)
      ...
      // Local prediction logic for corruption
      const result = (gdp * 0.1 + social * 0.2 + health * 0.1 + freedom * 0.1 + generosity * 0.1) * 0.7
      setPrediction(Number.parseFloat(result.toFixed(2)))
      setConfidence(75)
    } else if (targetVariable === "health") {
      // Local prediction logic for health
      const result = (gdp * 0.4 + social * 0.3 + freedom * 0.1 + generosity * 0.05 + corruption * 0.15) * 1.1
      setPrediction(Number.parseFloat(result.toFixed(2)))
      setConfidence(78)
    }
